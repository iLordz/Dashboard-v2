...
const Graphs2 = () => {
    ...
    const getAverageDurationByHour = (contactTimes) => {
        ...
        contactTimes.forEach(time => {
            const hour = time.getHours();
            hours[hour].total += parseInt(time.duration); // Asume que guardaste la duración
            hours[hour].count++;
        });
    ...
    return (
        <div style={{ padding: '10px', maxWidth: '1200px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', marginBottom: '10px', borderBottom: '1px solid #ddd', justifyContent: 'center', fontSize: '14px' }}>
                <button
                    style={{ padding: '10px 20px', background: activeTab === 'general' ? '#4a6da7' : 'transparent', color: activeTab === 'general' ? 'white' : '#333', border: 'none', cursor: 'pointer' }} onClick={() => setActiveTab('general')}>
                    Vista general
                </button>
                <button
            ...
            {activeTab === 'devices' && (
                <div style={{ backgroundColor: 'white', borderRadius: '10px', padding: '20px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
                    ...
                    {/* 3. Nueva gráfica: Duración promedio por hora */}
                    <div style={{ height: '400px' }}>
                        <Line
                            data={{
                                ...
                                datasets: Object.entries(groupByDevice()).map(([device, data]) => ({
                                    label: device,
                                    data: getAverageDurationByHour(data.contactTimes), // Nueva función
